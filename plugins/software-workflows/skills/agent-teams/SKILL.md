...
- **Codex Oracle**: Always uses the strongest OpenAI model at max reasoning depth automatically (auto-detected from the Codex CLI config) — `architect_review`, `code_review`, `research`, `codex_query`. Pass `infra: true` on any tool when the review needs LIVE state (SSH to servers, live DB queries, logs, dashboards) — read-only investigation; Codex discovers project access itself, but state the access pattern in the prompt when you know it.
