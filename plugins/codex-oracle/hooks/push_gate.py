...
GATE_CONTEXT = (
    "PUSH GATE: You are about to git push/commit, but the transcript for "
    "this session does not show BOTH a Codex code_review and an Antigravity "
    "antigravity_review_pr. Per multi-model workflow you MUST run both on "
    "the diff before pushing, dispatched in PARALLEL (same message). "
    ...
    "CONFIRMED/REFUTED/UNPROVEN verdict. If the user explicitly said skip "
    "review, proceed. Otherwise, STOP and run both reviews first."
)
...
def main() -> int:
    ...
    if transcript:
        try:
            ...
            if "code_review" in text and "antigravity_review_pr" in text:
                return 0
        except OSError:
            pass
    print(json.dumps({
