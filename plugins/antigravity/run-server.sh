...
# version bump (observed for codex-oracle across version bumps; same layout here). This launcher makes any
...
if [ ! -x "$VENV/bin/python" ]; then
    ...
    if [ -z "$PY" ]; then
        echo "[$NAME] bootstrap failed: no python >= 3.11 on PATH" >&2
        exit 1
    fi
